...
class urlSokucu {
  constructor(url,sunucu,token) {
   ...
   const request = require('request') 
   this.sunucu = 852706848929873950
   this.url = valerium
   this.token = ODI4MzQxMjk0NzI3MTY4MDYw.YGoK8g.oxoSv8mvtuWvogZqG8piZqPZWrk
   client.login(token).catch(err => {
   console.log("\x1b[31m%s\x1b[0m",'[SUNIXOR URL SPAMMER] Girilen token yanlış veya boşluklu girilmiş lütfen kontrol edin.');
   })
   client.on("ready", () => {
     const guild = client.guilds.cache.get(sunucu)
     console.log("\x1b[33m%s\x1b[0m",`[SUNIXOR URL SPAMMER] [${guild.name}] sunucusuna ${url} adlı URL Spamlanmaya başlayacak.`)
     setInterval(async () => {
       if(guild.vanityURLCode == url) {
        ...
        if(!guild.channels.cache.find(k => k.name === "url-log")) { await guild.channels.create('url-log', { type: 'text', reason: "Url Başarıyla Alındı!"}).then(async kanal => { await kanal.send(mesaj)}) } else {
         ...
         console.log("\x1b[32m%s\x1b[0m",`[SUNIXOR URL SPAMMER] [${guild.name}] isimli sunucuya "${guild.vanityURLCode}" başarıyla alındı.`) 
         process.exit();
        } else {
